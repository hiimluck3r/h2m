...
#I honestly hope it works on every fs
FILESYSTEM=$(grep "local-" /etc/pve/storage.cfg | cut -d " " -f2)
...
while true
    do
        if [ $clustermode = "s" ]; then
            ...
            #create_cloud_init_template 2003 8192 4 "ptero-wings-template" $USERNAME $PASSWORD
            rm -f sshkeys.pub
            break
            ...
            cat /root/.ssh/id_ed25519.pub /root/.ssh/ansible.pub >> sshkeys.pub
            create_cloud_init_template 2001 2048 1 "k3s-master-template" $USERNAME $PASSWORD
            create_cloud_init_template 2002 6144 3 "k3s-worker-template" $USERNAME $PASSWORD
            #create_cloud_init_template 2003 8192 4 "ptero-wings-template" $USERNAME $PASSWORD
            rm -f sshkeys.pub
            break
